...
def get_dead_links(soup, links):
    ...
    for index, link in enumerate(links):
        try:
            ...
            if not str(r.status_code).startswith("2") and \
                    not str(r.status_code).startswith("3"):
                dead_links = 1
    ...
    for tag in img_tags:
        if "width" in str(img_tags) and "height" in str(img_tags):
            ...
            # ExceptionType nicht an..
                ...
                try:
                    if round(float(im.size[0]) / im.size[1], 2)\
                            != round(float(tag.get("width"))
                                     / float(tag.get("height")), 2):
                        distorted_image = 1
...
def threads_images(soup, url):
    ...
    if img_tags:
        ...
        while beg < len(img_tags):
            t = Thread(target=get_distorted_images,
                       args=(url, soup, img_tags[beg:end]))
...
def get_w3c(url):
    ...
    if str(check_url.getcode()).startswith("2")\
            or str(check_url.getcode()).startswith("3"):
...
def get_factor(bad_fonts, bad_colors, fonts, marquee, gifs, bad_structure,
               gb, phrases, dead_links, audio, audio_loop, images, flash,
               popups, counter, w3):
    bad_fonts_max = 8
    bad_colors_max = 8
    font_amount_max = 5
    ...
    bad_structure_value = bad_structure_max if bad_structure else 0
    guestbook_value = guestbook_max if gb else 0
    ...
    audio_value = audio_max if audio else 0
    audio_loop_value = audio_loop_max if audio_loop else 0
    images_value = images_max if images else 0
    ...
    for site in websites:
        if site[2]:
            ...
            unusability = get_factor(bad_fonts, bad_colors, fonts, marquee,
                                     gifs, bad_structure, gb, phrases,
                                     dead_links, audio, audio_loop,
...
while end < num_of_websites:
    ...
    for website in websites_data:
        ...
        for x in website.items():
            update = session.query(Document).filter(Document.url ==
                                                    website.items()[
                                                        5][1])\
                .update({k: v for k, v in website.items()})
            session.commit()
